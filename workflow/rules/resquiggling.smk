...
rule f5c_eventalign:
    ...
    output:
        ...
        tsv=join("results", module_name, "f5c_eventalign2", "{cond}_{rep}_data.tsv"),
        summary=join("results", module_name, "f5c_eventalign2", "{cond}_{rep}_summary.tsv") 
    log: 
        # join("logs", module_name, rule_name, "{cond}_{rep}.log")
        join("logs", module_name, "f5c_eventalign2", "{cond}_{rep}.log")
    threads: get_threads(config, rule_name)
    params: opt=get_opt(config, rule_name)
    resources: mem_mb=get_mem(config, rule_name)
    container: f5c_container
