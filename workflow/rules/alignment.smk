...
rule minimap2_index:
    ...
    output: 
        ...
        idx=join("results", module_name, "minimap2_index2", "transcriptome_reference.mmi")
    log: 
        # join("logs",module_name, rule_name, "name.log")
        join("logs",module_name, "minimap2_index2", "name.log")
...
rule minimap2_align:
    input:
        ...
        index=rules.merge_fastq.output.fastq+".index" #place holder #should be removed after it's done
    output:
...
#         ref_list=join("results", module_name, rule_name, "cnot3_valid_references_list.txt"),
#     log: 
#         # join("logs",module_name, rule_name, "out.log")
#         join("logs",module_name, rule_name, "cnot3_out.log")
#     threads: get_threads(config, rule_name)
#     params: opt=get_opt(config, rule_name)
#     resources: mem_mb=get_mem(config, rule_name)
...
rule alignmemt_merge:
    input:
        ...
        bam_list=expand(join("results", module_name, "minimap2_align", "{{cond}}_{rep}.bam"), rep=replicates_list),
        bam_index_list=expand(join("results", module_name, "minimap2_align", "{{cond}}_{rep}.bam.bai"), rep=replicates_list) 
    output:
        # bam=join("results", module_name, rule_name, "{cond}.bam"),
        # bam_index=join("results", module_name, rule_name, "{cond}.bam.bai"),
        bam=join("results", module_name, "alignmemt_merge2", "{cond}.bam"),
        bam_index=join("results", module_name, "alignmemt_merge2", "{cond}.bam.bai"),
    log: 
        # join("logs",module_name, rule_name, "{cond}.log")
        join("logs",module_name, "alignmemt_merge2", "{cond}.log")
    threads: get_threads(config, rule_name)
    params: opt=get_opt(config, rule_name)
    resources: mem_mb=get_mem(config, rule_name)
    container: "library://aleg/default/minimap2:2.17"
