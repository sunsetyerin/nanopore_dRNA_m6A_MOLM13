...
rule nanocompore_sampcomp:
    input:
        ...
        # test_tsv=expand(join("results", module_name, "nanocompore_eventalign_collapse", "test_{rep}",
        test_tsv=expand(join("results", module_name, "nanocompore_eventalign_collapse", "cnot3_test_{rep}",  "out_eventalign_collapse.tsv"), rep=replicates_list),
    ...
    threads: get_threads(config, rule_name)
    params: opt=get_opt(config, rule_name),
            bed=get_bed(config,rule_name)
    resources: mem_mb=get_mem(config, rule_name)
    # container: "library://aleg/default/nanocompore:1.0.3"
    conda: "nanocompore" #/home/yekim/miniconda3/envs/nanocompore
...
rule nanocompore_postprocess:
    ...
    script: "../scripts/nanocompore_postprocess_sepsis.py"
