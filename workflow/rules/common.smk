...
def samples_load_validate(samplefile, schema):
    ...
    # This part was commented because of CNOT3 KD vs CNOT3 WT run
    ...
    # # rep2 = sorted(list(samples_df["replicate"][samples_df["condition"]=="cnot3_test"]))
    # if rep1 != rep2:
...
def fast5_files(wildcards): # custom-yerin
    file_list = expand(
        os.path.join(
            samples_df[(samples_df["condition"]==wildcards.cond)&(samples_df["replicate"]==int(wildcards.rep))]["fast5_dir"][0],
            "{fast5}.fast5"
            ),
...
def get_fastq_zip (wildcards): # custom-yerin
    res = samples_df[(samples_df["condition"]==wildcards.cond)&(samples_df["replicate"]==int(wildcards.rep))]
    return res["fastq_zip"][0]

def get_seqsum (wildcards): # custom-yerin
    res = samples_df[(samples_df["condition"]==wildcards.cond)&(samples_df["replicate"]==int(wildcards.rep))]
    return res["seqsum"][0]
...
def get_bed (config, rule_name, default=""):
    try:
        return config[rule_name]["bed"]
    except KeyError:
        logger.error("Could not find value `bed` for rule `{}` in config file".format(rule_name))
        return default
